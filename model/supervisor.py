...
from model.lr import linear_model_construction
from model.xgboost import xgboost_construction
...
class EncoderDecoder():
    ...
    def __init__(self, is_training=True, **kwargs):
        ...
        if self._type == 'lstm_ed':
            ...
            self._input_dim_linear = self._data['encoder_input_train'].shape[1]*self._data['encoder_input_train'].shape[2]
            if is_training:
                self.model = linear_model_construction(self._input_dim_linear, self._output_dim, self._optimizer, self._log_dir, is_training=is_training)
            else:
                self.model = linear_model_construction(self._input_dim_linear, self._output_dim, self._optimizer, self._log_dir, is_training=is_training)
        elif self._type == 'xgboost':
            if is_training:
                self.model = xgboost_construction(is_training, self._log_dir)
            else:
                self.model = xgboost_construction(is_training, self._log_dir)
        else:
    ...
    def train(self):
        if self._type == 'lstm_ed' or self._type == 'bilstm_ed':
        ...
        if self._type == 'linear':
            self._data['encoder_input_train'] = self._data['encoder_input_train'].reshape(self._data['encoder_input_train'].shape[0] , -1)
            self._data['encoder_input_val'] = self._data['encoder_input_val'].reshape(self._data['encoder_input_val'].shape[0] , -1)
            self.model.compile(optimizer=self._optimizer, loss='mse', metrics=['mse', 'mae'])
        ...
        if self._type == 'xgboost':
            ...
            self._data['encoder_input_train'] = self._data['encoder_input_train'].reshape(self._data['encoder_input_train'].shape[0] , -1)
            self._data['encoder_input_val'] = self._data['encoder_input_val'].reshape(self._data['encoder_input_val'].shape[0] , -1)
            self._data['decoder_target_train']=self._data['decoder_target_train'].reshape(self._data['decoder_target_train'].shape[0],-1)
            self._data['decoder_target_val']=self._data['decoder_target_val'].reshape(self._data['decoder_target_val'].shape[0],-1)

            eval_set=[ (self._data['encoder_input_train'], self._data['decoder_target_train']),(self._data['encoder_input_val'],self._data['decoder_target_val'])]
            
            self.model.fit(self._data['encoder_input_train'],
                            self._data['decoder_target_train'],
                            eval_set=eval_set,
                            eval_metric='rmse',
            ...
            #log val loss , loss , config
            training_history = {}
            training_history['loss'] = self.model.evals_result()['validation_0']['rmse']
            training_history['val_loss'] = self.model.evals_result()['validation_0']['rmse']
            ...
            filename = self._log_dir + "best_model.sav"
    ...
    def _predict(self, source):
        ...
        if self._type == 'linear' or self._type == 'xgboost':

            source = source.reshape(source.shape[0], -1)
            
            output = self.model.predict(source)
            preds[0] = output[0]
